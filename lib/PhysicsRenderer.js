function PhysicsRenderer( size , shader , renderer ){
  ...
  this.toCorrect = this.rt_1.clone();
...
PhysicsRenderer.prototype.createCorrectionProgram = function(){
 ...
 // var fs = shaders.setValue( shaders.fs.hexCorrection , 'SIZE' , this.size + "." );
  ...
  var uniforms = {
    ...
    radius: { type: "f"   , value: 10/this.size },
    

    blurRatio: { type: "f"   , value: .999999 },
...
PhysicsRenderer.prototype.FSZero = [
  "void main() {",
  "  gl_FragColor = vec4(0.) ;",
  "}"
].join("\n");
...
PhysicsRenderer.prototype.FSBlur = [
  "uniform sampler2D texture;",
...
PhysicsRenderer.prototype.update = function(){
  ...
  if( flipFlop == 0 ){
    ...
    this.pass( this.simulation , this.toCorrect );
    this.correct( this.toCorrect , this.rt_3 );
    ...
    this.correct( this.toCorrect , this.rt_1 );
    ...
    this.pass( this.simulation , this.toCorrect );
    this.correct( this.toCorrect , this.rt_2 );
...
PhysicsRenderer.prototype.correct = function( texture , target ){

  this.correctionProgram.uniforms.texture.value = texture;
  this.pass( this.correctionProgram , target );
